...
export default function AdminSaffron() {
  ...
  const handleUploadImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const filePath = `saffron/img_${Date.now()}.${ext}`;
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let error;
      if (selectedId) {
        ({ error } = await supabase.from("saffron").update(payload).eq("id", selectedId));
      } else {
        ({ error } = await supabase.from("saffron").insert([payload]));
      }
      if (error) throw error;
      await fetch("/api/revalidate?path=/saffron");
      closeModal();
      fetchItems();
    } catch (err: any) {
  ...
  return (
    <div className="admin-page">
      <div className="admin-page-header">
        <div>
          <h1 className="admin-page-title">Saffron</h1>
          <p className="admin-page-subtitle">Manage the banner and variety cards for the Saffron page.</p>
        </div>
      </div>
