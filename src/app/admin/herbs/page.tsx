...
export default function AdminHerbs() {
  ...
  const handleUploadImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const filePath = `herbs/img_${Date.now()}.${ext}`;
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let error;
      if (selectedId) {
        ({ error } = await supabase.from("herbs").update(payload).eq("id", selectedId));
      } else {
        ({ error } = await supabase.from("herbs").insert([payload]));
      }
      if (error) throw error;
      await fetch("/api/revalidate?path=/herbs");
      closeModal();
      fetchItems();
    } catch (err: any) {
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      const { data, error } = await supabase.from("herbs").delete().eq("id", id).select();
      if (error) throw error;
  ...
  return (
    <div className="admin-page">
      <div className="admin-page-header">
        <div>
          <h1 className="admin-page-title">Herbs</h1>
          <p className="admin-page-subtitle">Manage the banner and variety cards for the Herbs page.</p>
        </div>
      </div>
