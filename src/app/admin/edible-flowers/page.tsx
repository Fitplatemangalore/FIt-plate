...
export default function AdminEdibleFlowers() {
  ...
  const handleUploadImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const filePath = `edible_flowers/img_${Date.now()}.${ext}`;
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let error;
      if (selectedId) {
        ({ error } = await supabase.from("edible_flowers").update(payload).eq("id", selectedId));
      } else {
        ({ error } = await supabase.from("edible_flowers").insert([payload]));
      }
      if (error) throw error;
      await fetch("/api/revalidate?path=/edible-flowers");
      closeModal();
      fetchItems();
    } catch (err: any) {
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      const { data, error } = await supabase.from("edible_flowers").delete().eq("id", id).select();
      if (error) throw error;
  ...
  return (
    <div className="admin-page">
      <div className="admin-page-header">
        <div>
          <h1 className="admin-page-title">Edible Flowers</h1>
          <p className="admin-page-subtitle">Manage the banner and variety cards for the Edible Flowers page.</p>
        </div>
      </div>
