...
export default function AdminFruits() {
  ...
  const handleUploadImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const filePath = `fruits/img_${Date.now()}.${ext}`;
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let error;
      if (selectedId) {
        ({ error } = await supabase.from("fruits").update(payload).eq("id", selectedId));
      } else {
        ({ error } = await supabase.from("fruits").insert([payload]));
      }
      if (error) throw error;
      await fetch("/api/revalidate?path=/fruits");
      closeModal();
      fetchItems();
    } catch (err: any) {
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      const { data, error } = await supabase.from("fruits").delete().eq("id", id).select();
      if (error) throw error;
  ...
  return (
    <div className="admin-page">
      <div className="admin-page-header">
        <div>
          <h1 className="admin-page-title">Fruits</h1>
          <p className="admin-page-subtitle">Manage the banner and variety cards for the Fruits page.</p>
        </div>
      </div>
